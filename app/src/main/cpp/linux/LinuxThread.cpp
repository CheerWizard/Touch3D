...
//    LogAssert(result != 0, "Failed to set thread affinity mask on Linux!");

    // Set priority
    sched_param param {};
    param.sched_priority = T3D_THREAD_PRIORITY_CODE[m_priority];
    result = pthread_setschedparam(m_handle, SCHED_OTHER, &param);
//    LogAssert(result != 0, "Failed to set thread priority on Linux!");

    // Set name
    result = pthread_setname_np(m_handle, m_name);
//    LogAssert(result != 0, "Failed to set thread name on Linux!");
}